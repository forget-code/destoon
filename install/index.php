...
switch($step) {
	...
	case '4'://数据库
		...
		$CFG['cookie_pre'] = 'c'.strtolower(random(2)).'_';
		$percent = '60%';
		include IN_ROOT.'/step_'.$step.'.tpl.php';
	break;
	case '5'://安装进度
			...
			if($setting) {
				...
				if($i == 3) {
					foreach($setting as $k=>$v) {
						$setting[$k] = str_replace('http://www.destoon.com/', $CFG['url'], $v);
					}
				}
	...
	case '6'://安装成功
		$percent = '100%';
		include IN_ROOT.'/step_'.$step.'.tpl.php';
		file_put(DT_CACHE.'/install.lock', time());
		$index = file_get(DT_ROOT.'/index.html');
		if(strpos($index, 'install/') === false) {
			file_put(DT_ROOT.'/install/index.php', '<meta http-equiv="Cache-Control" content="no-cache"/><meta http-equiv="refresh" content="0;url=../?install=success">');
		} else {
			file_del(DT_ROOT.'/index.html');
			file_put(DT_ROOT.'/install/index.php', '<meta http-equiv="Cache-Control" content="no-cache"/><meta http-equiv="refresh" content="0;url=../index.php?install=success">');
		}
	break;
	case 'db_test':
		...
		if(is_array($tables) && in_array($ttb_pre."company_setting", $tables)) {
			if($ttb_test) {
				exit('<script>alert("数据库设置正确，连接正常\n\n注意：系统检测到您已经安装过Destoon，如果继续安装将会清空现有数据\n\n如果需要保留现有数据，请修改数据表前缀");</script>');
			} else {
				exit('<script>alert("警告：系统检测到您已经安装过Destoon，如果继续安装将会清空现有数据\n\n如果需要保留现有数据，请修改数据表前缀");</script>');
			}			
		}
