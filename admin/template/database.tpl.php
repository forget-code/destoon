...
<td class="px11" title="数据:<?php echo $v['size'];?> 索引:<?php echo $v['index'];?>"><?php echo $v['tsize'];?></td>
<td class="px11"><?php echo $v['chip'];?></td>
<td><a href="?file=<?php echo $file;?>&action=optimize&tables=<?php echo $v['name'];?>">优化</a> | <a href="?file=<?php echo $file;?>&action=repair&tables=<?php echo $v['name'];?>">修复</a> | <a href="?file=<?php echo $file;?>&action=export&table=<?php echo $v['name'];?>">下载</a> | <a href="###" onclick="Dict('<?php echo $v['name'];?>','<?php echo urlencode($v['note']);?>');">字典</a></td>
</tr>
<?php }?>
</table>
<?php if($tables) {?>
...
function Dict(t, n) {
	mkDialog('', '<iframe src="?file=tag&action=dict&table='+t+'&note='+n+'" width="600" height=300" border="0" vspace="0" hspace="0" marginwidth="0" marginheight="0" framespacing="0" frameborder="0" scrolling="yes"></iframe>', '数据字典', 620, 0, 0);
}
</script>
